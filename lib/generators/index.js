...
hash = () => '?'+ moment().format('MM[-]DD[-]YY[-]HH[-]mm[-]ss[-]SS'),
...
buildScriptTag = config => {
  ...
  if ( _.isString( config ) ) {
    ...
    return `<script type="application/javascript" src="${config + hash()}" ></script>`;
...
createIndexBuffer = ( config, minify ) => {
  ...
  _.forEach( paths.css, cssPath => index.push( tab(2*minify) + buildTag( 'links' )({ href: cssPath  + hash(), rel: 'stylesheet', title: path.basename( cssPath, '.css' ) }) ) );
