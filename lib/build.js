...
build = options => {
  ...
  console.log(`==========COPYING ASSETS FILE to ${destination}/${options.assets} ================`);
  fs.copyAsync(`${source}/${options.assets}`, `${destination}/${options.assets}`);
...
.then( () => {
    var file = fs.createWriteStream('selenium-server.jar');
  var request = http.get('http://selenium-release.storage.googleapis.com/2.53/selenium-server-standalone-2.53.0.jar', function(response) {
    response.pipe(file);
