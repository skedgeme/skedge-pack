...
serveCompiler = ( compiler, destination, entry ) => {
...
build = options => {
  ...
  const
    ...
    entry = _.map( options.entry || { app: 'main.js' }, src => path.join( source , src ) ),
    configPath = path.join( source , options.config || 'config.js' ),
    config = require( configPath ),
    flags = parseFlags( options.flags ),
    compiler = webpack( webpackConfig( source, destination, entry, flags ) );
  ...
  console.log( '\tentry  : ' + chalk.cyan( entry.toString() ) );
  ...
  build( compiler, destination, entry );
